  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-1">eSewa Fine Payment</h3>
        <p className="text-xs text-slate-500 mb-4">Ticket ID: <span className="font-semibold text-slate-800">{ticket.ticketNumber}</span> | Fine: <span className="font-bold text-emerald-600">Rs. {ticket.fineAmount}</span></p>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center mb-5">
          <div className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-800 uppercase tracking-wide mb-2">
            <QrCode className="w-4 h-4" /> Scan Official eSewa QR Code
          </div>
          {qrUrl ? (
            <div className="bg-white p-3 rounded-lg inline-block shadow border border-emerald-100">
              <img src={qrUrl} alt="Official eSewa Payment QR" className="w-48 h-48 object-contain mx-auto" />
            </div>
          ) : (
            <div className="p-6 bg-slate-100 rounded-lg text-slate-500 text-xs">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="border-2 border-dashed border-slate-300 rounded-xl p-4 text-center hover:border-emerald-500 transition-colors cursor-pointer relative bg-slate-50">
              ...
              {previewUrl ? (
                <div>
                  ...
                  <span className="text-xs text-emerald-600 font-semibold block">{proofFile?.name}</span>
                </div>
              ) : (
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-lg shadow flex items-center gap-1.5 transition-all disabled:opacity-50"
            >
