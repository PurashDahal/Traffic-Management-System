    ...
    <div className="space-y-6">
      <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-extrabold flex items-center gap-2">
            <Car className="w-7 h-7 text-emerald-400" /> Welcome, {user?.fullName}
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <span className="text-3xl font-extrabold text-slate-900 block mt-1">{vehicles.length}</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-rose-200 shadow-sm bg-rose-50/40">
          ...
          <span className="text-3xl font-extrabold text-rose-900 block mt-1">{unpaidTickets.length}</span>
          <span className="text-[10px] text-rose-700 block mt-1 font-semibold">Total Fine: Rs. {totalUnpaidFine.toLocaleString()}</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-amber-200 shadow-sm bg-amber-50/40">
          ...
          <span className="text-3xl font-extrabold text-amber-900 block mt-1">{pendingTickets.length}</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-emerald-200 shadow-sm bg-emerald-50/40">
          ...
          <span className="text-3xl font-extrabold text-emerald-900 block mt-1">{paidTickets.length}</span>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="bg-rose-50 border border-rose-200 p-4 rounded-2xl flex items-center justify-between text-rose-900">
            <div className="flex items-center gap-2 text-xs font-bold">
              ...
              <span>You have {unpaidTickets.length} unpaid fine(s). Scan the official eSewa QR code to complete payment.</span>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-sm text-slate-900 flex items-center gap-2">
          <FileText className="w-4 h-4 text-emerald-600" /> All Violation Tickets ({tickets.length})
        </h3>
